...
export default function SettingsProfilePage() {
  ...
  // Frontend-only: Load sub-vendors from localStorage or use mock data
  const fetchSubVendors = useCallback(async () => {
    ...
    try {
      ...
      if (storedVendors) {
        ...
        // Default mock data
        vendors = [
          { id: 1, name: 'Ahmed Ali', email: 'ahmed.ali@example.com', status: 'Active', role: 'Vendor Manager' },
          { id: 2, name: 'Fatima Khan', email: 'fatima.khan@example.com', status: 'Active', role: 'Product Manager' },
          { id: 3, name: 'Hassan Raza', email: 'hassan.raza@example.com', status: 'Pending', role: 'Order Manager' }
        ]
      ...
      setSelectedRow(vendors.length ? vendors[0].id : null)
      setCurrentPage(1)
    } catch (error) {
      ...
      try {
        ...
        if (vendorProfile) {
          ...
          if (profileImageUrl) {
            ...
            const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(nameToUse)}&size=120&background=random`
  ...
  useEffect(() => {
    if (subVendors.length > 0 && typeof window !== 'undefined') {
                    ...
                    {paginatedVendors.map((vendor: SubVendor, index: number) => {
                      ...
                      <tr
                        ...
                        onClick={() => {
                          ...
                          navigate(`/settings/profile/vendors/${vendor.id}`)
                        }}
