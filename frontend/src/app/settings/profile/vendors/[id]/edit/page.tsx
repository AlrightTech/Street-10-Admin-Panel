...
export default function EditSubVendorPage() {
  ...
  const fetchVendor = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/sub-admin/users/${vendorId}`)
      ...
      const statusFromApi = typeof user.status === 'string' ? user.status : 'Active'
      const normalizedStatus = (() => {
        const lower = statusFromApi.toLowerCase()
      ...
      setFormData({
        fullName: user.name || user.fullName || defaultFormState.fullName,
        email: user.email || defaultFormState.email,
        role: user.role || defaultFormState.role,
        status: normalizedStatus
      })

      const incomingPermissions = user.permissions || {}

      setPermissions({
        viewProducts: Boolean(incomingPermissions.viewProducts),
        editProducts: Boolean(incomingPermissions.editProducts),
        deleteProducts: Boolean(incomingPermissions.deleteProducts),
        approveProducts: Boolean(incomingPermissions.approveProducts)
      })
    } catch (error) {
